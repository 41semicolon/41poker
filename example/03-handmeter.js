...
const { handmeter } = require('../src/hand.js');
...
console.log('preflop', handmeter([], hands).map(x => `${x.toFixed(0)}%`));
console.log('flop   ', handmeter(board3, hands).map(x => `${x.toFixed(0)}%`), 'Borad:', board3.map(repr));
console.log('turn   ', handmeter(board4, hands).map(x => `${x.toFixed(0)}%`), 'Board:', board4.map(repr));
